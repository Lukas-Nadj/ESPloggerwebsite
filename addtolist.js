...
function deleteItem(index) {
  ...
  const url ="http://"+document.getElementById("source").value +"/delete?filename=" + itemToDelete.getElementsByTagName("p")[0].innerHTML;
  console.log(url);
  const xhr = new XMLHttpRequest();
  xhr.open("GET", url);
  xhr.onload = function () {
    if (xhr.status === 200) {
      const trimmedUrl = document.getElementById("source").value.replace(/^https:\/\//i, "");
